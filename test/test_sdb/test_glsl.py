...
def test_make_sbd_exprs():
    ...
    assert sdb.gen_getSDB(s4, ids) == dedent("""\
        float getSDB4(in vec4 x) {
            float dp;
            float d = getSDB0(x);
            dp = getSDB1(x);
            if (dp > d) d = dp;
    ...
    assert sdb.gen_getSDB(s5, ids) == dedent("""\
        float getSDB5(in vec4 x) {
            return max(getSDB1(x), -getSDB2(x));
        }\n\n""")

    s = sdb.Hemisphere(0.5, (1.0, 2.0, 3.0), -1)
    assert sdb.gen_getSDB(s, {s:0}) == dedent("""\
        float getSDB0(in vec4 x) {
            return min(sphereSDF(vec3(1.0, 2.0, 3.0), 0.5, x.xyz), -1*(x.z - 3.0));
        }\n\n""")


    s = sdb.ZemaxConic(0.1, 0.05, 1., 0.9, [0.1, 0.2], [0.5, 0.6, 0.7])
