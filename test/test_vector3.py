...
def test_make_rotation():
    for vector in ([1, 0, 0], [0, 1, 0], [0, 0, 1], v4b.normalize([1, -1, 2])):
...
def test_dot():
    assert np.isreal(v4b.dot([1, 2, 3], [1, 2, 3]))
    testing.assert_array_equal(v4b.dot([1, 2, 3], [1, 2, 3]), 14)
    testing.assert_array_equal(v4b.dot([1+1j, 2, 3], [1-1j, 2, 3]), 13+2j)
    testing.assert_array_equal(v4b.dot([1, 2, 3]), 14)

    a = np.arange(3) + np.arange(4)[:, None]
    b = np.arange(3) + np.arange(4)[:, None] + np.arange(5)[:, None, None]*1j
    testing.assert_array_equal(v4b.dot(a, b), (a*b.conj()).sum(-1, keepdims=True))
