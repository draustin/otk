...
def make_MLA150(small:bool=False):
    ...
    w = 10e-3
    h = 10e-3
    unitfn = ZemaxConicSagFunction(roc, d/2)
    fn = RectangularArraySagFunction(unitfn, (p, p))
    front = Sag(fn, 1)
    sides = InfiniteRectangularPrism(w, h)
    back = Plane((0, 0, 1), -t)
    surface = IntersectionOp((front, sides, back))
    element = SimpleElement(surface, ri.fused_silica, lambda x: perfect_refractor)
    return element

element = make_MLA150()
assembly = Assembly([element], UniformIsotropic(ri.air))
