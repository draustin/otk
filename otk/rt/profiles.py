...
class Profile:
    ...
    def calc_z(self, x, y):
        ...
        """
        z = self.intersect(vector3.stack_xyzw(x, y, 0, 1), np.asarray([0, 0, 1, 0]))[..., 0]
...
class SphericalProfile(Profile):
    ...
    def calc_normal(self, point):
        ...
        normal[..., 2] += 1
        normal[..., 3] = 0
        return normal  # nx = -point[..., 0]/self.roc  # ny = -point[..., 1]/self.roc  # nz = (self.roc - point[..., 2])/self.roc  # return vector3.stack_xyzw(nx, ny, nz, 0)
...
class ArbitraryRotationallySymmetricProfile(Profile):
    ...
    def intersect(self, origin, vector):
        ...
        while num_iterations < self.max_num_iterations:
            ...
            drhodd = 2*(vector3.dot(point[..., :2], vector[..., :2]) + d*vector3.dot(vector[..., :2]))
    ...
    def calc_normal(self, point):
        ...
        return vector3.normalize(vector3.stack_xyzw(-point[..., 0]*factor, -point[..., 1]*factor, 1,
            0))  # return vector3.normalize(vector3.stack_xyzw(-point[0]*factor, -point[1]*factor, 1, 0))
...
class LatticeProfile(Profile):
    ...
    def intersect(self, origin, vector):
        ...
        center = vector3.stack_xyzw(cx, cy, 0, 1)
    ...
    def make_sections(self, partitions: Sequence[Tuple[Tuple[float, float], Tuple[float, float]]], origin: np.ndarray,
        ...
        for (ix, iy), (d1, d2) in partitions:
            ...
            section = vector3.stack_xyzw(x, y, z, 1)
...
class SphericalSquareArrayProfile(SquareArrayProfile):
    ...
    #     cx, cy = self.calc_center(nx, ny)
    #     return vector3.intersect_spherical_surface(ox - cx, oy - cy, oz, vx, vy, vz, self.roc)[..., None]
    #
    # def calc_center(self, nx, ny):
    #     return (nx + 0.5)*self.pitch, (ny + 0.5)*self.pitch
    #
    ...
    #     return vector3.stack_xyzw(normal_x, normal_y, normal_z, 0)
