...
class Mode:
    ...
    def __init__(self, line_base: rt1.Line, line1: rt1.ComplexLine, line2: rt1.ComplexLine):
    ...
    def unstack(cls, line):
        ...
        bundle_base = rt1.Line(line.origin[0, ...], line.vector[0, ...])
        bundle1 = rt1.ComplexLine(rt1.Line(line.origin[1, ...], line.vector[1, ...]),
                                  rt1.Line(line.origin[2, ...], line.vector[2, ...]))
        bundle2 = rt1.ComplexLine(rt1.Line(line.origin[3, ...], line.vector[3, ...]),
                                  rt1.Line(line.origin[4, ...], line.vector[4, ...]))
    ...
    def make(cls, line_base, axis1, lamb, waist1, d1=0, waist2=None, d2=None):
        ...
        axis2 = v4hb.normalize(v4hb.cross(vector, axis1))
        axis1 = v4hb.cross(axis2, vector)
        theta1 = lamb / (np.pi * waist1)
        theta2 = lamb / (np.pi * waist2)

        # Define axis 1 divergence ray.
        vector1_real = v4hb.normalize(vector + axis1 * theta1)
        ...
        vector2_real = v4hb.normalize(vector + axis2 * theta2)
        ...
        bundle1 = rt1.ComplexLine(rt1.Line(origin1_real, vector1_real), rt1.Line(origin1_imag, vector1_imag))
        bundle2 = rt1.ComplexLine(rt1.Line(origin2_real, vector2_real), rt1.Line(origin2_imag, vector2_imag))

        return cls(line_base, bundle1, bundle2), (axis1, axis2)

    def project_profile(self, profile):
        points = v4hb.stack_xyzw(profile.x, profile.y, profile.z, 1)
    ...
    def make_angular_grid(cls, lamb, waist, theta_mins, theta_maxs, num_rays):
        ...
        origin = 0, 0, 0, 1
        bundle = cls.make(rt1.Line(origin, vector), axis1, lamb, waist)
        return bundle
...
def follow_pbg_bundle_trace(ray_trace, lamb, phi_axis_origin=None, all_inside=None):
    ...
    for intersection in ray_trace.intersections:
        ...
        current_bundle = rt1.Line(intersection.point, intersection.deflected_vector)
...
def calc_lagrange_invariant(h1, u1, h2, u2):
    ...
    """
    return v4hb.dot(u1, h2) - v4hb.dot(u2, h1)
...
def calc_pbg_field(k, base, h1origin, u1, h2origin, u2, point, flux=1, phi_axis_origin=0, calc_grad_phi=False):
    ...
    a = v4hb.triple(v, u1, u2)  # z**2 coefficient.
    b = v4hb.triple(v, u1, h2origin) + v4hb.triple(v, h1origin, u2)  # z coefficient
    h1_cross_h2_origin = v4hb.triple(v, h1origin, h2origin)  # constant
    h1_cross_h2 = z * (a * z + b) + h1_cross_h2_origin

    u1_dot_r = v4hb.dot(u1, r)
    u2_dot_r = v4hb.dot(u2, r)
    v_h1_r = v4hb.triple(v, h1, r)
    v_h2_r = v4hb.triple(v, h2, r)
    ...
    if calc_grad_phi:
        ...
        # (and likewise for h2).
        dw_numerator_dz = v4hb.triple(v, u1, r) * u2_dot_r - v4hb.triple(v, u2, r) * u1_dot_r
        ...
        gradr_w_numerator = v_h1_r * u2 + v4hb.cross(v, h1).conj() * u2_dot_r - (
                    v_h2_r * u1 + v4hb.cross(v, h2).conj() * u1_dot_r)
...
def calc_edge_distance(k, base, h1origin, u1, h2origin, u2, z, rhat, exponent=1):
    ...
    a = v4hb.triple(base.vector, u1, u2)  # z**2 coefficient.
    b = v4hb.triple(base.vector, u1, h2origin) + v4hb.triple(base.vector, h1origin, u2)  # z coefficient.
    cross_origin = v4hb.triple(base.vector, h1origin, h2origin)  # Constant coefficient.
    cross = z * (a * z + b) + cross_origin

    # Complex phase is quadratic in displacement along rhat. Get coefficient.
    wk = k * (v4hb.triple(base.vector, h1, rhat) * v4hb.dot(u2, rhat) - v4hb.triple(base.vector, h2, rhat) * v4hb.dot(u1,
                                                                                                                  rhat)) / (
                 2 * cross)
