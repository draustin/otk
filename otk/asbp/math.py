...
#
#     The central transverse wavenumber is qc. The propagator that should be applied is -f*q**2*z/(2*k)
#     (no zeroth order term).
#
#     Returns:
#         f: factor by which propagation distance should be multiplied to give quadratic propagation distance
#         delta_kz: Zero order correction factor. The corresponding additional phase is delta_kz*z.
#         delta_gkz: first order correction factor i.e. result should have phase of delta_gkz*q*z applied. Negative of
#             the corresponding translation.
#     """
#     # Get zeroth, first and second order (nonmixed) derivatives of k at (kxc, kyc).
#     kz, gkz, _, g2kz, _, _ = expand_kz(k, qc, 0)
#
...
# @dataclass
# class PlaneToCurvedSSTArbitrary1D:
#     QinvTP: Array2D # Product of propagator, inverse DFT and quadratic output factor. Axes are (ro, q).
#     gradQinvTP: Array2D # Partial derivative of QinvTP w.r.t r.
#     T: Array2D # DFT factor. Axes are (q, ri).
#     Qi: Array2D # Input quadratic phase factor. Axes are (ro, ri).
#
#     def __post_init__(self):
#         len_ro, len_ri = self.QinvTP.shape
#         assert self.gradQinvTP.shape == (len_ro, len_ri)
#         assert self.T.shape == (len_ri, len_ri)
#         assert self.Qi.shape == (len_ro, len_ri)
#
#     def eval(self, Eri: Array1D):
#         # ro: i,  q: k,  ri: m.
#         Ero = opt_einsum.contract('ik, km, im, m -> i', self.QinvTP, self.T, self.Qi, Eri)
#         gradxEro = opt_einsum.contract('ik, km, im, m -> i', self.gradQinvTP, self.T, self.Qi, Eri)
...
def calc_plane_to_curved_sst_arbitrary_factors_1d(
    ...
    xi = sa.calc_r(r_support, num_points, ri_center)
    kxi = sa.calc_q(r_support, num_points, q_center)  # +q_curvatures)

    # Quadratic phase is centered at origin. Numerically, this requires evaluation of complex exponentials of size N^3.
    # Significant fraction of total cost of this function, but unavoidable.
    Qix = calc_quadratic_phase_1d(-k, xi[:, 0] - ri_center[0], roc_x[:, :, None])
...
def prepare_plane_to_curved_sst_arbitrary(k, rs_support, num_pointss, z, xo, yo, roc_xo, roc_yo, rs_center=(0, 0),
                                          qs_center=(0, 0), ro_centers=None, kz_mode='local_xy'):
    ...
    """
    invTPx, gradxphiinvTPx, invTPy, gradyphiinvTpy, Tx, Ty, Qix, Qiy = \
        calc_plane_to_curved_sst_arbitrary_factors(
            k, rs_support, num_pointss, z, xo, yo, roc_xo, roc_yo, rs_center, qs_center, ro_centers, kz_mode)
    x_factor = opt_einsum.contract('ijk, km, ijm -> ijm', invTPx, Tx, Qix)
    y_factor = opt_einsum.contract('ijl, ln, ijn -> ijn', invTPy, Ty, Qiy)
