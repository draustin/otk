...
@singledispatch
def calc_amplitudes(self: Interface, n0: float, n1:float, cos_theta0: float, lamb: float) -> tuple:
    raise NotImplementedError()
...
def deflect_ray(interface: Interface, ray: Ray, x0:np.ndarray, x1:np.ndarray, dielectric0: np.ndarray, normal: np.ndarray,
    dielectric1: np.ndarray, element1: Element) -> tuple:
    ...
    amplitudes = calc_amplitudes(interface, n0, n1, cos_theta0, ray.lamb)
    ...
    rays = []
    if not all(a == 0 for a in amplitudes[0]):
    ...
    if not all(a == 0 for a in amplitudes[1]):
...
def _get_transformed_element(self: Assembly, x: Sequence4) -> Element:
    ...
    return element.transform(root_to_local)
...
def _process_ray(self: Assembly, ray:Ray, sphere_trace_kwargs:dict) -> tuple:
    ...
    if element0 is None:
        ...
        medium1 = element1.medium
        interface = element1.interface
        #tensor1 = element1.get_dielectric_tensor(ray.lamb, trace.x)
        normal = -backend.getnormal(element1.surface, trace.xm) # Normal points from 0 to 1.
        if logger.isEnabledFor(logging.DEBUG):
            logger.debug(f'{trace} - {element1.surface}')
    else:
        medium0 = element0.medium
        trace = backend.spheretrace(element0.surface, ray.line.origin, ray.line.vector, sign=-1., through=True,
            **sphere_trace_kwargs)
        if trace.d <= 0:
            if trace.steps >= sphere_trace_kwargs['max_steps']:
                warn(f"Spheretrace reached max_steps={sphere_trace_kwargs['max_steps']}.")
        ...
        normal = backend.getnormal(element0.surface, xm)
        #deflector = element0.get_deflector(x0)
        interface = element0.interface
        ...
        if element1 is None:
            ...
            normal1 = -backend.getnormal(element1.surface, xm)
            assert np.allclose(normal, normal1)
            # TODO which interface to use?
            if logger.isEnabledFor(logging.DEBUG):
                logger.debug(f'{element0.element.surface} - {trace} - {element1.element.surface}')

    ray = ray.advance(trace.tm)
    tensor0 = get_dielectric_tensor(medium0, ray.lamb, x0)
    tensor1 = get_dielectric_tensor(medium1, ray.lamb, x1)
    deflected_rays = deflect_ray(interface, ray, x0, x1, tensor0, normal, tensor1, element1)
